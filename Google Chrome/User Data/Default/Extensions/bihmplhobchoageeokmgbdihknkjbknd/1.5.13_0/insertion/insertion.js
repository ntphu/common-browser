...
},{"./Port":17}],15:[function(require,module,exports){
'use strict';
...
},{"uuid":12}],16:[function(require,module,exports){
'use strict';
...
},{"./ForegroundHub":14,"./Pipe":15}],17:[function(require,module,exports){
"use strict";
...
var PageInterface = (function () {
  function PageInterface() {
    ...
    _classCallCheck(this, PageInterface);

    this.status = {};
    this.domain = this.parseUrl(document.location.href).domain;

    _WdgtStorage2['default'].get(SETTINGS).then(function (settings) {
      if (settings && settings.pageInterface === false) return;

      _WdgtStorage2['default'].get(DATA).then(function (data) {
        if (!data) {
          data = {
            x: ['right', ((1 - (window.innerWidth - 15) / window.innerWidth) * 100).toFixed(8) + '%'],
            y: ['bottom', ((1 - (window.innerHeight - 15) / window.innerHeight) * 100).toFixed(8) + '%']
          };
        }
        _this.init(data);
      });
  ...
  _createClass(PageInterface, [{
    key: 'init',
    value: function init(data) {
      var _this2 = this;

      chrome.proxySDK.networkConfiguration.getCountries().then(function (countries) {
        _this2.countries = countries;
      });
      this.getStatus().then(function (status) {
        _this2.status.mode = status.mode;
        _this2.status.status = status.status;
        _this2.status.country = _this2.getCountry(status.country);

        _React2['default'].domReady().then(function () {
          _React2['default'].insertInBegining(_this2.render(), document.body);
          chrome.proxySDK.proxy.onStatusChanged.add(_this2.statusChanged.bind(_this2));
          _this2.setEventsListeners();
          _this2.setIconPosition(data);
        });
      });
    }
  }, {
    ...
    value: function setEventsListeners() {
      _React2['default'].getElement('touchvpn_icon_container').addEventListener('dragstart', (function (e) {
        var style = this.getIconComputedPosition();
        this.deltaX = e.clientX - style.x[1];
        this.deltaY = e.clientY - style.y[1];
      }).bind(this));

      _React2['default'].getElement('touchvpn_icon_container').addEventListener('dragend', (function (e) {
        var x = e.clientX - e.view.screenLeft - this.deltaX;
        var y = e.clientY - e.view.screenTop - this.deltaY;
        var position = {
          x: ['left'],
          y: ['top']
        };

        position.x[1] = (x / window.innerWidth * 100).toFixed(8);
        position.y[1] = (y / window.innerHeight * 100).toFixed(8);

        if (x < 0) {
          position.x[1] = '0%';
        } else if (x > window.innerWidth - ICON_WIDTH) {
          position.x[0] = 'right';
          position.x[1] = '0%';
        } else {
        ...
        if (y < 0) {
          position.y[1] = '0%';
        } else if (y > window.innerHeight - ICON_HEIGHT) {
          position.y[0] = 'bottom';
          position.y[1] = '0%';
        } else {
        ...
        _WdgtStorage2['default'].set(DATA, position);
        this.setIconPosition(position);
      }).bind(this), false);
    }
  }, {
    key: 'setIconPosition',
    value: function setIconPosition(position) {
      var x = position.x[0] + ': ' + position.x[1] + ' !important;',
          y = position.y[0] + ': ' + position.y[1] + ' !important;';

      _React2['default'].getElement('touchvpn_icon_container').setAttribute('style', x + ' ' + y);
    }
  }, {
    key: 'getIconComputedPosition',
    value: function getIconComputedPosition() {
      var parsePosition = function parsePosition(position) {
        return Number(position.match(/[0-9.]+/)[0]);
      };

      var computed_style = window.getComputedStyle(_React2['default'].getElement('touchvpn_icon_container'), null);
      var position = {
        x: [],
        y: []
      };
      ...
      if (computed_style.left != 'auto') {
        position.x[1] = parsePosition(computed_style.left);
      } else {
        position.x[1] = window.innerWidth - ICON_WIDTH - parsePosition(computed_style.right);
      }

      position.y[0] = 'top';
      if (computed_style.top != 'auto') {
        position.y[1] = parsePosition(computed_style.top);
      } else {
        position.y[1] = window.innerHeight - ICON_HEIGHT - parsePosition(computed_style.bottom);
      }
    ...
    key: 'statusChanged',
    value: function statusChanged(status) {
      var _this3 = this;

      if (status.status == this.status.status) {
        return;
      }

      if (status.status == 'connecting' && (status.domain == this.domain || status.domain == 'global')) {
        _React2['default'].getElement('touchvpn_icon_container').setAttribute('class', status.status);
        this.status.status = status.status;
        return;
      }

      this.getStatus().then(function (status) {
        _this3.status.mode = status.mode;
        _this3.status.status = status.status;
        _this3.status.country = _this3.getCountry(status.country);
        _React2['default'].getElement('touchvpn_icon_container').setAttribute('class', status.status);
        _React2['default'].getElement('touchvpn_status_icon').setAttribute('class', 'touchvpn_' + _this3.status.country);
      });
    }
  }, {
    key: 'getCountries',
    value: function getCountries() {
      ...
      while (j < 4) {
        if (this.countries[i] != 'us') {
          countries.push(_React2['default'].createElement(
            'div',
            { 'class': "touchvpn_country touchvpn_" + this.countries[i], click: this.connect.bind(this, this.countries[i]) },
            _React2['default'].createElement(
              'div',
              { 'class': 'touchvpn_icon_sign' },
              'Browse from ',
              _countryData2['default'].countries[this.countries[i].toUpperCase()].name
            )
          ));
          j++;
        }
    ...
    key: 'connect',
    value: function connect(country) {
      chrome.proxySDK.proxy.reconnect({ mode: 'global', domain: this.domain, country: country });
    }
  }, {
    key: 'getStatus',
    value: function getStatus() {
      return chrome.proxySDK.proxy.getStatus(this.domain);
    }
  }, {
    key: 'parseUrl',
    ...
    value: function render() {
      return _React2['default'].createElement(
        'div',
        { id: 'touchvpn_icon_container', 'class': this.status.status, draggable: 'true', style: 'left:' + (window.innerWidth - 80) + 'px !important; bottom:15px !important;' },
        _React2['default'].createElement(
          'div',
          { id: 'touchvpn_status_icon', 'class': "touchvpn_" + this.status.country, click: this.connect.bind(this, 'optimal') },
          _React2['default'].createElement(
            'div',
            { 'class': 'touchvpn_icon_sign' },
            'Best choice for browsing'
          )
        ),
        _React2['default'].createElement(
          'div',
          { id: 'touchvpn_icon_selector' },
          this.getCountries()
        )
      );
    }
...
exports['default'] = PageInterface;
module.exports = exports['default'];

},{"../React":19,"Wdgt/Storage":13,"country-data":6}],19:[function(require,module,exports){
'use strict';
...
var _PageInterfacePageInterface = require('./PageInterface/PageInterface');

var _PageInterfacePageInterface2 = _interopRequireDefault(_PageInterfacePageInterface);
...
var insertion = function insertion() {
  ...
  if (!document.location.href.match(/^https?/) || document.location.href.indexOf('chrome/newtab') != -1) {
    return;
  }

  new _PipeOut2['default']('SDK').then(function (sdk) {
    chrome.proxySDK = sdk;
    new _PageInterfacePageInterface2['default']();
  });
};

new insertion();

},{"./PageInterface/PageInterface":18,"PipeOut":16}]},{},[20]);
