var replayobj = {
    ...
    readFragments: function () {
        ...
        var currentTime = replayobj.player.getCurrentTime();
        if(replayobj.startSeekTime < currentTime && replayobj.endSeekTime < currentTime) {
    ...
    createHTML5Player: function (videoEle) {
        ...
        return {
            getDuration: function () {
                return Math.floor(videoEle.duration);
            },
    ...
    afixReplayButton: function (buttons) {
        if (!replayobj.videoEle) {
            ...
            if (!htmlParent.length) {
                htmlParent = $("#player #player-container");
            }
            if (!htmlParent.length) {
